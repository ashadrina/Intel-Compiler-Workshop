#!/bin/bash
#
# clean_all.sh - Remove build artifacts from all Lab 4 solution directories
#
# This script cleans object files, binaries, and optimization reports
# from all numbered solution directories (00-baseline through 07-combined)
#
# Usage: ./clean_all.sh
#
...
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  Lab 4 - Cleaning All Build Artifacts                     ║"
echo "╚════════════════════════════════════════════════════════════╝"
echo ""
...
for dir in "${SOLUTIONS[@]}"; do
    ...
    # Count and remove IPO/no-IPO binaries (from 01-unit-stride)
    if [ -f "$dir/matvec_ipo" ]; then
        removed_count=$((removed_count + 1))
        rm -f "$dir/matvec_ipo"
    fi

    if [ -f "$dir/matvec_noipo" ]; then
        removed_count=$((removed_count + 1))
        rm -f "$dir/matvec_noipo"
    fi
